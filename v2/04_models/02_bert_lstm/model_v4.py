...
class ECHR2_model(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Batch normalization
        self.bn1 = nn.BatchNorm1d(self.num_str_vars)
...
#        for idx in range(0, self.max_n_pars):
        ...
        x = self.lstm_case_doc(x)                                       # Tuple (len = 2)
        x_fwd = x[0][:, -1, 0:self.h_dim]                               # batch_size x hidden_dim
        x_bkwd = x[0][:, 0, self.h_dim:self.h_dim*2]                    # batch_size x hidden_dim
