...
def train_epoch_f(args, epoch, model, criterion_1, 
                  criterion_2, optimizer, train_dl,
    ...
    for idx, (X_art, X_case, X_viol_pars, Y) in tqdm(enumerate(train_dl),
                                                    total = len(train_dl),
                                                    desc = 'Training epoch'):
        ...
        pred, masked_alpha_2, masked_alpha_3 = model(X_art, X_case)
        pred = pred.view(-1)
...
def val_epoch_f(model, criterion_1, criterion_2, dev_dl, device):
    model.eval()
    sum_correct = 0
    sum_val_loss = 0
    total_entries = 0
...
###
    ...
    # Instantiate optimizer & criterion_1
    ...
    for epoch in tqdm(range(args.n_epochs), desc = 'Training dataset'):
        ...
        train_loss, train_acc = train_epoch_f(args, epoch, model, criterion_1,
                                              criterion_2, optimizer, train_dl,
                                              output_train_log_file_path, device)
        ...
        val_loss, val_acc = val_epoch_f(model, criterion_1, criterion_2,
                                        dev_dl, device)
