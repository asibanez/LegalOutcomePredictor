...
class LM_model(nn.Module):
    def __init__(self, args, pretrained_embeddings_leaf_nodes,
        ...
        # Max pooling tree
        self.max_pool_tree = nn.MaxPool1d(kernel_size = self.num_leaf_nodes)
    ...
    def forward(self, X_str, X_loss_desc, X_leaf_node_texts):
        ...
        # Tree max pooling
        x_tree = x_tree.transpose(1,2)                                 # batch_size x (hidden_dim x 2) x n_leaf_nodes
        x_tree = self.max_pool_tree(x_tree)                            # batch_size x (hidden_dim x 2) x 1
        x_tree = x_tree.squeeze(2)                                     # batch_size x (hidden_dim x 2)
