...
def train_epoch_f(gpu, args):
    ...
    # Instantiate model
    str_vars = model_train.columns.drop(['LOSS_DESCRIPTION', 'LEAF_NODE_TEXTS', 'full_denial'])
    ...
    for idx_ep, epoch in enumerate(tqdm(range(args.n_epochs))):        
        ...
        for idx, (X_str,X_loss_desc, X_leaf_node_texts, Y) in tqdm(enumerate(train_dl),
                                                                   total = len(train_dl),
                                                                   desc = 'Training'):    
            # Write log file
            if gpu == 0:
            ...
            pred = model(X_str, X_loss_desc, X_leaf_node_texts).view(-1)
...
def validate_f(model, args, criterion, dev_dataset, device, val_loss_history, val_acc_history):
    ...
    for X_str, X_loss_desc, X_leaf_node_texts, Y in tqdm(
