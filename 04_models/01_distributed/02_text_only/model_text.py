...
class LM_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return X_loss_desc, X_leaf_node_texts, Y

# Model definition
class LM_model(nn.Module):
    def __init__(self, args, pretrained_embeddings_leaf_nodes, pretrained_embeddings_loss_desc):
        ...
        # Fully connected concat(loss_desc, tree) -> Sigmoid
    ...
    def forward(self, X_loss_desc, X_leaf_node_texts):
        bilstm_b = self.h_dim
        ...
        # Query vector
        query_v = self.fc_query(x_loss_desc).unsqueeze(2)              # batch_size x att_dim x 1
        ...
        for idx in range(0, self.num_leaf_nodes):
            ...
            x_single_leaf_node = x_leaf_nodes[:, span_b:span_e, :]
            ...
            alpha = torch.bmm(projection, query_v)                     # batch_size x seq_len x 1
        ...
        # Tree encoding
        self.lstm_tree.flatten_parameters()
        x_tree = self.lstm_tree(x_tree)                                # Tuple (len = 2)
        x_tree_fwd = x_tree[0][:, -1, 0:bilstm_b]                      # batch_size x hidden_dim
        x_tree_bkwd = x_tree[0][:, 0, bilstm_b:bilstm_e]               # batch_size x hidden_dim
        x_tree = torch.cat((x_tree_fwd, x_tree_bkwd), dim = 1)         # batch_size x (hidden_dim x 2)
        x_tree = self.drops(x_tree)                                    # batch_size x (hidden_dim x 2)
        
        # Concatenate loss description and tree encodings
        x = torch.cat((x_loss_desc, x_tree), dim = 1)                  # batch size x (hidden_dim x 2 x 2)
