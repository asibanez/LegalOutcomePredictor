...
class LM_model(nn.Module):
    def __init__(self, args, pretrained_embeddings_leaf_nodes,
        ...
        self.bn_output = nn.BatchNorm1d(self.h_dim*2 + self.h_dim*2*35 + int(self.num_str_vars/3))
        self.fc_output = nn.Linear(in_features = self.h_dim*2 + self.h_dim*2*35 + int(self.num_str_vars/3),
    ...
    def forward(self, X_str, X_loss_desc, X_leaf_node_texts):
        ...
        x_tree = x_tree.flatten(start_dim = 1)                         # batch_size x (n_leaf_nodes x hidden_dim x 2)
        ...
        x = torch.cat((x_loss_desc, x_tree, x_str), dim = 1)           # batch size x (hidden_dim x 2 + hidden_dim x 2 x num_leaf_nodes + num_str_vars / 3)
        x = self.bn_output(x)                                          # batch size x (hidden_dim x 2 + hidden_dim x 2 x num_leaf_nodes + num_str_vars / 3)
