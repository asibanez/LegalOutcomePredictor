# v2 -> Stores texts instead of IDs
...
output_path_train = os.path.join(base_folder, '01_data', '01_preprocessed', 'train_df.csv')
output_path_dev = os.path.join(base_folder, '01_data', '01_preprocessed', 'dev_df.csv')
output_path_test = os.path.join(base_folder, '01_data', '01_preprocessed', 'test_df.csv')
...
for case in tqdm.tqdm(case_test_df.iterrows()):
    ...
    selected_case_texts = [select_passage(num_passages_per_case, case_texts)] * 66
    ...
    aux_df = pd.DataFrame({'article_text':art_text,
                           'case_texts': selected_case_texts,
                           'outcome': outcome})
...
str_data_train.to_csv(output_path_train, index = False)
str_data_dev.to_csv(output_path_dev, index = False)
str_data_test.to_csv(output_path_test, index = False)
