...
# v14 -> Generates labels for violated paragraphs
...
def dataset_preproc_f(dataset_df, ECHR_art_df, ECHR_par_df, selected_arts,
    ...
    str_data = pd.DataFrame(columns = ['case_id', 'article_id', 'article_pars_ids',
                                       'case_texts_ids', 'violated_pars', 'outcome'])
    ...
    for _, case in tqdm.tqdm(dataset_df.iterrows(), total = len(dataset_df)):
        ...
        violated_pars = case['VIOLATED_PARAGRAPHS']
        ...
        # Iterate over violated article list and updated corresponding outcome label
        ...
        aux_df = pd.DataFrame({'case_id': case_id_list,
                               ...
                               'violated_pars': violated_art_pars_list,
...
x_train = pd.DataFrame(str_data_train[['case_id', 'article_id', 'article_pars_ids',
                                       'violated_pars', 'case_texts_ids']])
